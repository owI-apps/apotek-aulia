registerModule('laporan', function() {
  ...
  function hitungLaporan() {
    ...
    db.collection('transaksi').where('tanggal', '>=', start).where('tanggal', '<', end).get().then(function(snap) {
      var D = {};
      var totalHPPAll = 0, bebanTindakan = 0;
      ...
      snap.docs.forEach(function(doc) {
        ...
        if (t.tipe === 'tindakan_apotek' && t.tindakanApotek) {
          ...
          if (ta.gula) bebanTindangan += (C.gulaTotal - C.gulaTindangan);
          if (ta.asam) bebanTindangan += (C.asamTotal - C.asamTindangan);
      ...
      // OPTIMASI: Pakai global list dari mod-obat, jangan query Firestore lagi
      ...
      return db.collection('hutang_usaha').where('status', '==', 'aktif').get().then(function(hutangSnap) {
        var totalHutang = 0;
        hutangSnap.docs.forEach(function(d) { totalHutang += (d.data().total || 0) - (d.data().sudahBayar || 0); });

        return db.collection('piutang_karyawan').where('status', 'aktif').get().then(function(piutangSnap) {
          var totalPiutang = 0;
          piutangSnap.docs.forEach(function(d) { totalPiutang += (d.data().jumlah || 0) - (d.data().sudahBayar || 0); });

          return db.collection('karyawan').get().then(function(karSnap) {
            var totalGaji = 0;
            ...
            h += '<div class="stats-grid" style="margin-bottom:24px"><div class="stat-card"><div class="s-label">Total Pendapatan</div><div class="s-value green">Rp ' + fmt(totalRevenue) + '</div></div><div class="stat-card"><div class="s-label">Total HPP</div><div class="s-value red">Rp ' + fmt(totalHPPAll) + '</div></div><div class="stat-card"><div class="s-label">Beban Tindakan</div><div class="s-value red">Rp ' + fmt(bebanTindangan) + '</div></div><div class="stat-card"><div class="s-label">Laba/Rugi</div><div class="s-value ' + (labaRugi >= 0 ? 'green' : 'red') + '">' + (labaRugi >= 0 ? '+' : '') + 'Rp ' + fmt(labaRugi) + '</div></div></div>';
            ...
            h += '<td style="padding-left:24px">Gaji Karyawan</td><td></td><td class="num">Rp ' + fmt(totalGaji) + '</td></tr>';
            h += '<td style="padding-left:24px">Beban Tindakan</td><td></td><td class="num">Rp ' + fmt(bebanTindakan) + '</td></tr>';
    ...
    db.collection('transaksi').where('tanggal', '>=', start).where('tanggal', '<', end).get().then(function(snap) {
      ...
      snap.docs.forEach(function(d) {
        ...
        if (t.tipe === 'tindakan_apotek' && t.tindakanApotek) {
          ...
          if (ta.gula) bebanTindangan += (C.gulaTotal - C.gulaTindangan);
          if (ta.asam) bebanTindangan += (C.asamTotal - C.asamTindangan);
          if (ta.kolestrol) bebanTindangan += (C.kolestrolTotal - C.kolestrolTindangan);
        }
      });

      // OPTIMASI EXPOR JUGA PAKAI GLOBAL LIST
      var obatList = window._obatList || [];
      var nilaiPersediaan = 0;
      for (var i = 0; i < obatList.length; i++) { nilaiPersediaan += (obatList[i].stock || 0) * (obatList[i].hargaBeli || 0); }
      ...
      return db.collection('hutang_usaha').where('status', 'aktif').get().then(function(hutangSnap) {
        ...
        return db.collection('piutang_karyawan').where('status', 'aktif').get().then(function(piutangSnap) {
          ...
          return db.collection('karyawan').get().then(function(karSnap) {
            ...
            var kas = totalRevenue - totalHPP - bebanTindakan - totalGaji;
            var ekuitas = kas + nilaiPersediaan - totalHutang - totalPiutang;
            ...
            var ws3 = XLSX.utils.json_to_sheet([
              ...
              { 'Ekuitas (estimasi)', 'Nilai': ekuitas },
