registerModule('laporan', function() {
  ...
  function hitungLaporan() {
    ...
    db.collection('transaksi').where('tanggal','>=',start).where('tanggal','<',end).get().then(function(snap) {
      ...
      return db.collection('obat').get().then(function(obatSnap) {
        var nilaiPersediaan = 0;
        obatSnap.docs.forEach(function(d) { nilaiPersediaan += (d.data().stock||0) * (d.data().hargaBeli||0); });
        ...
        return db.collection('hutang_usaha').where('status','==','aktif').get().then(function(hutangSnap) {
          ...
          return db.collection('piutang_karyawan').where('status','==','aktif').get().then(function(piutangSnap) {
            ...
            return db.collection('karyawan').get().then(function(karSnap) {
              ...
              var ekuitas = kas + nilaiPersediaan - totalHutang - totalPiutang;
              var labaRugi = totalRevenue - totalHPP - bebanTindangan;
