registerModule('pengaturan', function() {
  ...
  function render() {
    page.innerHTML = `
      ...
      <div class="settings-panel active" id="tab-umum">
        ...
        <div class="settings-section"><h3>Margin Obat Resep</h3>
          <div class="settings-grid">
            <div class="form-group"><label>Margin (%)</label><input type="number" id="cfg-margin" min="0" step="0.1" max="100" value="35"></div>
          </div>
          <p style="font-size:12px;color:var(--muted);margin-top:8px">Harga jual = HPP × (1 + margin%). Contoh: HPP 10.000, margin 35% → Rp 13.500</p>
        </div>
      </div>
      ...
      <div class="settings-panel" id="tab-resep">
        <div class="settings-section"><h3>Resep Klinik (per resep)</h3>
          ...
          <div class="skema-ref"><h4>Pembagian Bagian Klinik</h4>
            <table>
        ...
        <div class="settings-section"><h3>Racik</h3>
          ...
          <p style="font-size:12px;color:var(--muted);margin-top:8px">Biaya per item obat yang diracik. Pembagian mengikuti skema D.a</p>
        </div>
      </div>
      ...
      <div class="settings-panel" id="tab-tindakan">
        ...
        <div class="settings-section"><h3>Tindakan Apotek</h3>
          <div class="settings-grid">
            <div class="form-group"><label>Gula Darah - Total (Rp)</label><input type="number" id="cfg-gulaTotal" min="0"></div>
            <div class="form-group"><label>Gula Darah - Fee (Rp)</label><input type="number" id="cfg-gulaTindakan" min="0"></div>
            <div class="form-group"><label>Asam Urat - Total (Rp)</label><input type="number" id="cfg-asamTotal" min="0"></div>
            <div class="form-group"><label>Asam Urat - Fee (Rp)</label><input type="number" id="cfg-asamTindakan" min="0"></div>
            <div class="form-group"><label>Kolestrol - Total (Rp)</label><input type="number" id="cfg-kolestrolTotal" min="0"></div>
            <div class="form-group"><label>Kolestrol - Fee (Rp)</label><input type="number" id="cfg-kolestrolTindakan" min="0"></div>
          </div>
          <div class="skema-ref"><h4>Pembagian Tindakan Apotek</h4>
            <table>
              <tr><th>Tindakan</th><th>Total</th><th>Fee (D.a)</th><th>HPP Alat</th></tr>
              <tr><td>Cek Gula Darah</td><td>Rp ${fmt(C.gulaTotal)}</td><td>Rp ${fmt(C.gulaTindakan)}</td><td>Rp ${fmt(C.gulaTotal - C.gulaTindangan)}</td></tr>
              <tr><td>Cek Asam Urat</td><td>Rp ${fmt(C.asamTotal)}</td><td>Rp ${fmt(C.asamTindakan)}</td><td>Rp ${fmt(C.asamTotal - C.asamTindakan)}</td></tr>
              <tr><td>Cek Kolestrol</td><td>Rp ${fmt(C.kolestrolTotal)}</td><td>Rp ${fmt(C.kolestrolTindakan)}</td><td>Rp ${fmt(C.kolestrolTotal - C.kolestrolTindakan)}</td></tr>
            </table>
          </div>
        </div>
      </div>
      ...
      <div class="settings-panel" id="tab-payroll">
        <div class="settings-section"><h3>Gaji Pokok (kini di menu Karyawan)</h3>
          <p style="font-size:12px;color:var(--muted);margin-bottom:14px">Gaji pokok dikelola per karyawan di menu Karyawan. Field di bawah readonly untuk referensi.</p>
          <div class="settings-grid">
        ...
        <div class="settings-section"><h3>Tunjangan</h3>
          ...
          <div class="skema-ref"><h4>Skema D.a</h4>
            <table>
    ...
    document.getElementById('btnResetConfig').addEventListener('click', () => {
      if (confirm('Reset pengaturan ke default?')) { C = { ...DC }; configToForm(); toast('Direset ke default','warning'); }
    });
  }
