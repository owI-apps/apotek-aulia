registerModule('pasien', function() {
      ...
      <div class="table-wrap">
        <div style="overflow-x:auto">
          <table>
            ...
            <tbody id="psBody">
              <tr><td colspan="${canWrite ? 8 : 7}"><div class="empty-state"><i class="fas fa-user-injured"></i><p>Memuat...</p></div></td></tr>
            </tbody>
          </table>
        </div>
        <div class="table-footer"><span id="psCount">0 data</span></div>
      </div>`;
  ...
  function startListener() {
    db.collection('pasien').orderBy('noRM','desc').onSnapshot(snap => {
      ...
      // PENTING: Expose ke global supaya modul Rawat & Transaksi bisa pakai (hemat kuota)
      window._pasienList = allPasien; 
      
      renderTable();
    }, err => console.error('Pasien listener error:', err));
  }

  function renderTable() {
    const s = (document.getElementById('psSearch').value || '').toLowerCase();
    const f = allPasien.filter(p => {
      if (!s) return true;
      return (p.noRM || '').toLowerCase().includes(s) || 
             (p.nama || '').toLowerCase().includes(s) || 
             (p.telepon || '').includes(s);
    });
    ...
    if (!f.length) {
      tb.innerHTML = `<tr><td colspan="${canWrite ? 8 : 7}"><div class="empty-state"><i class="fas fa-user-injured"></i><p>${allPasien.length ? 'Tidak ditemukan.' : 'Belum ada data pasien.'}</p></div></td></tr>`;
      return;
    }
    ...
    tb.innerHTML = f.map((p, i) => `
      <tr>
        <td>${i + 1}</td>
        <td style="font-family:var(--mono);font-size:12px;color:var(--accent)">${p.noRM || '-'}</td>
        <td style="font-weight:500">${p.nama || '-'}</td>
        <td style="font-size:12px">${p.jenisKelamin || '-'}</td>
        <td style="font-family:var(--mono);font-size:12px">${p.nik || '-'}</td>
        <td style="font-size:12px">${p.telepon || '-'}</td>
        <td style="font-size:11px;color:var(--danger);max-width:120px;overflow:hidden;text-overflow:ellipsis">${(p.alergi || []).join(', ') || '-'}</td>
        ${canWrite ? `<td class="actions">
          <button class="btn btn-outline btn-xs" onclick="PS.edit('${p.id}')"><i class="fas fa-edit"></i></button>
          <button class="btn btn-danger btn-xs" onclick="PS.hapus('${p.id}', '${escAttr(p.nama)}')"><i class="fas fa-trash"></i></button>
        </td>` : ''}
      </tr>
    `).join('');
  }
  ...
  async function openAdd() {
    ...
    // Generate Nomor RM Anti Duplikat
    try {
      var nomorRM = await generateNomor('RM');
      document.getElementById('psNoRM').value = nomorRM;
    } catch(e) {
      document.getElementById('psNoRM').value = 'RM-0001';
    }
  ...
  async function hapus(id, nama) { 
    ...
    try { 
      await db.collection('pasien').doc(id).delete(); 
  ...
  document.getElementById('psBtnSave').addEventListener('click', async () => {
    ...
    try {
      if (eid) { 
        await db.collection('pasien').doc(eid).update(d); 
        toast('Diperbarui', 'success'); 
      } else { 
        d.createdAt = now(); 
        await db.collection('pasien').add(d); 
